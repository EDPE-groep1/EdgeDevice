...
def StoreSensorData(device_id,temperature, humidity, pressure, date):
    try:
        ...
        curs = db.cursor()
        sql = "Insert into DataPi (device_id, temperature, humidity, date) Values(%s,%.5s,%.5s,%s)"
        val = (device_id,temperature, humidity, date)
...
def GetSensorData():
    ...
    try:
        ...
        for reading in curs.fetchall():
            items.append({'Device_ID': str(reading[0]),'temperature': str(reading[1]), 'humidity': str(reading[2]), 'timestamp': str(reading[3])})
        curs.fetchall()
